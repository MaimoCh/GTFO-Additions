...
import gtfoaddn.modules.GTFOAddnModuleManager;
...
@Mod(modid = GTFOAddnValues.MODID,
        name = GTFOAddnValues.MODNAME,
        version = Tags.VERSION,
        dependencies = "required-after:mixinbooter;" +
                GTInternalTags.DEP_VERSION_STRING + "required-after:" + GTFOAddnValues.MODID_GTFO + ";")
public class GTFOAddnMod {

    private GTFOAddnModuleManager moduleManager;

    @Mod.EventHandler
    public void onConstruction(FMLConstructionEvent event) {
        MinecraftForge.EVENT_BUS.register(this);
        GTFOAddnLog.logger.info("starting construction event...");
        moduleManager = GTFOAddnModuleManager.getInstance();
        moduleManager.registerContainer(new GTFOAddnModules());
        moduleManager.setup(event.getASMHarvestedData(), Loader.instance().getConfigDir());
        moduleManager.onConstruction(event);
        GTFOAddnLog.logger.info("finished construction!");
    }
    ...
    public void registerBlocks(RegistryEvent.Register<Block> event) {
        GTFOAddnLog.logger.info("Registering Blocks...");
        moduleManager.registerBlocks(event);
    }

    @SubscribeEvent
    public void registerItems(RegistryEvent.Register<Item> event) {
        GTFOAddnLog.logger.info("Registering Items...");
    ...
    public static <T extends Block> ItemBlock createItemBlock(T block, Function<T, ItemBlock> producer) {
        ...
        if (registryName == null) {
            GTFOAddnLog.logger.error("Block has no registry name: {}", block.getTranslationKey(), new Throwable());
        } else {
    ...
    public static void syncConfigValues(ConfigChangedEvent.OnConfigChangedEvent event) {
        if (event.getModID().equals(GTFOAddnValues.MODID)) {
            ConfigManager.sync(GTFOAddnValues.MODID, Config.Type.INSTANCE);
        }
    }
}
